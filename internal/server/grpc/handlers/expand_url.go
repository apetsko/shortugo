...
func (h *Handler) Expand(ctx context.Context, req *pb.ExpandRequest) (*pb.ExpandResponse, error) {
	originalURL, err := h.URLHandler.Storage.Get(ctx, req.ShortUrlId)
	if err != nil {
		switch {
		case errors.Is(err, shared.ErrGone):
			h.URLHandler.Logger.Error("URL is gone: " + req.ShortUrlId)
			return nil, status.Error(codes.FailedPrecondition, "URL is gone")

		case errors.Is(err, shared.ErrNotFound):
			h.URLHandler.Logger.Error("URL not found: " + req.ShortUrlId)
